...
|[Basic encoding with H264 with Event Hub/Event Grid](/VideoEncoding/Encoding_H264%20_with_EventHub/index.ts)| Shows how to use the standard encoder and receive and process Event Grid events from Media Services through an Event Hub. You must first setup an Event Grid subscription that pushes events into an Event Hub using the Azure Portal or CLI to use this sample. |
...
| [Encode a multi-channel audio source file](/VideoEncoding/Encoding_MultiChannel_Audio/) | This sample demonstrates how to create an encoding Transform that uses channel mappings and audio track selection from the input source to output two new AAC audio tracks.
