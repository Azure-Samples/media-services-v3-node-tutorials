...
shaka.util.Id3Utils = class {
    ...
    static getFrameData_(data) {
        ...
        const size = dataView.getInt32(4); // pass in the offset of 4
    ...
    static decodeFrame_(frame) {
        ...
        if (frame.type === 'TXXX') {
            ...
            const text = StringUtils.fromUTF8(frame.data.subarray(1)).replace(
                /\0*$/,
                ''
            );
             ...
             * [0-?] = {URL}
             */
            const url = StringUtils.fromUTF8(frame.data).replace(/\0*$/, '');
            ...
            if (
                frame.data[0] !== shaka.util.Id3Utils.UTF8_encoding &&
                frame.data[0] != 0
            ) {
                console.log.warning(
            ...
            const data = StringUtils.fromUTF8(
                BufferUtils.toUint8(frame.data, 4 + mimeTypeEndIndex)
            ).replace(/\0*$/, '');
    ...
    static getID3Frames(id3Data) {
        console.log('Getting the ID3 frames');
