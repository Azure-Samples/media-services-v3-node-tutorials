...
export async function main() {
  ...
  try {
    ...
    if (outputAsset.name !== undefined) {
      ...
      // Now we are going to "late bind" add an additional WebVTT file to this asset. 
      ...
      // By using SAS URLs we dont need the acccount keys in this code. 
      ...
      // Language: en-us
      // 
      let filename= "sample.vtt"
      let webVttFile = `Media\\${filename}`;

      console.log("Uploading the VTT file to the output asset.")
      // Now we have a SAS URL to write back into the output asset. This is where we are going to upload our WebVTT file to add to the output asset
      if (outputAssetContainerSas.assetContainerSasUrls) {
        let uploadSasUrl = outputAssetContainerSas.assetContainerSasUrls[0];
        uploadFileToSas(uploadSasUrl,webVttFile);
      }
      ...
      // Caveats, and more information on how to update VTT files.
      // If you want to edit and update the above VTT file, you must call the 
      // mediaServicesClient.tracks.beginUpdateTrackDataAndWait method instead of beginCreateOrUpdateAndWait()

      // If you have to update any of the HLS settings on the VTT file, you should call beginCreateOrUpdateAndWait()

      // One of the primary restrictions of the API is that you cannot rename the vtt file once it has been registered as a track on the asset.
      // You can only update it with the beginUpdateTrackDataAndWait method. 
